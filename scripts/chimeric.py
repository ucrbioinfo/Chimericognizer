...
def output_allalms(alms_list, filename):
    fo = file(filename, 'w')
    fo.write("optmap_id\tref_id\tqry_id\torientation\tconfidence\tstart\tend\n")
    for opt_id in range(len(alms_list)):
        alms = alms_list[opt_id]
        for ref in alms:
            for qry in alms[ref]:
                x = alms[ref][qry]
                fo.write(str(opt_id)+"\t"+str(ref)+"\t"+str(qry)+"\t"+str(x.orientation)+"\t"+str(x.confidence)+"\t"+str(x.start)+"\t"+str(x.end)+"\n")
    fo.close()
...
def build_graph(vertices, vid_vertex, vertex_vid, edges, chimeric_pairs, optmap_i, num, mergedist):
    for i in range(0, len(chimeric_pairs)):
        ...
        if ref_id not in vertices:
            ...
            for p in vertices[ref_id]:
                if abs(p-ref_pos) <= mergedist: # if the distance of 2 candidate cutting sites is smaller than 80,000bp, merge them
                    found = True
                    ref_pos_found = p
                    break
            if found == False:
        ...
        if qry_id not in vertices:
            ...
            for p in vertices[qry_id]:
                if abs(p-qry_pos) <= mergedist: # if the distance of 2 candidate cutting sites is smaller than 80,000bp, merge them
                    found = True
                    qry_pos_found = p
                    break
            if found == False:
...
def chimeric(myfile_list, myfile2_list, output_dir, ref_quality, qry_quality, min_confidence, minrefoverhang, minqryoverhang, mergedist):
    ...
    output_allalms(all_alms_list, output_dir+"/alms_used.log")
    ...
    for optmap_i in range(len(chimeric_pairs_list)):
        ...
        newnum = build_graph(vertices, vid_vertex, vertex_vid, edges, chimeric_pairs, optmap_i, num, mergedist)
        num = newnum
    ...
    vid_weight = vertex_weights(all_alms_list, vid_vertex, ugraph, minrefoverhang, minqryoverhang, ref_quality, qry_quality)
    ...
    for ug in ugraphs:
        ...
        if len(ug) <= 20:
            ...
            cover = vertex_cover_aprx.VC(ug, sub_vid_weight)
        for v in cover:
            whole_cover.add(v)
    ...
    ref_cuts = {}
    for optmap_i in range(len(myfile_list)):
        ref_cuts[optmap_i] = {}
