class Alignment:
    def __init__(self, qry, ref, qrystartpos, qryendpos, refstartpos, refendpos, orientation, confidence, hitenum, qrylen, reflen, channel, alignmentstring): 
        ...
        self.ref_left_overlen = 0
        self.ref_right_overlen = 0
        self.chimeric = False # will be change later
        self.contained = False # will be change later

    def __str__(self):
        return 'ref %d refstart %.1f refend %.1f qry %d qrylen %.1f qrystart %.1f qryend %.1f %s conf %.2f start %.2f end %2.f qry_left_overlen %2.f qrt_right_overlen %2.f chimeric %s contained %s' % (self.ref, self.refstartpos, self.refendpos, self.qry, self.qrylen, self.qrystartpos, self.qryendpos, self.orientation, self.confidence, self.start, self.end, self.refstart, self.refend, self.qry_left_overlen, self.qrt_right_overlen, self.chimeric, self.contained)
